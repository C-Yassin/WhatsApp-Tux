...
class GetSystemGUI:
    def __init__(self, parent=None):
        # Check the initial dark mode status
    ...
    def is_dark_mode_enabled_linux_mint(self):
        try:
            # Check the current GTK theme setting
            cmd = ['gsettings', 'get', 'org.cinnamon.desktop.interface', 'gtk-theme']
            result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            
            # Dark themes often contain 'dark' in the theme name
    ...
    def handle_link_install(self, link):
        if link:
            ...
            if "pin.it" in link:
                ...
                # Rebuild the clean URL using only the scheme, netloc, and the first two path segments
                path_parts = parsed_url.path.split('/')
                
                # Ensure that the URL has the expected structure
                if len(path_parts) > 3 and path_parts[1] == "pin":
                    # Construct the cleaned URL (keeping only the first two path parts: /pin/{id})
...
class WebAppViewer(QMainWindow):
    def __init__(self):
        ...
        # Load permissions to check if About dialog was already shown
        # Check if the dialog has been shown, if not show it once
        ...
        if self.check_internet():
            ...
            # Set up timer to check unread message count periodically
    ...
    def show_settings_window(self, widget):
        ...
        if not self.settings_window:
            ...
            # Descriptive text for password section
            ...
            # Separator for a clean visual break
            separator = Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL)
            vbox.pack_start(separator, False, False, 10)

            # Notifications section with description and checkbox
            ...
            # Change Download Directory Section
            ...
            # Smaller "Change" button on the right
            ...
            # Clear Cookies Button with Red Warning
        ...
        # Warning text for clearing cookies
            ...
            # Define the CSS for normal and hover states
            css = b"""
                .warning-label {
    ...
    def choose_directory(self, widget):
        ...
        if response == Gtk.ResponseType.OK:
            # Update the download directory and save it
            self.download_directory = dialog.get_filename()
            self.save_download_directory()

            # Update the label to reflect the new directory
    ...
    def on_toggle_password(self, checkbox):
        ...
        if checkbox.get_active():
            if os.path.exists(PASSWORD_FILE):
                # If password file exists, prompt to reset the password
                ...
                if response == Gtk.ResponseType.OK:
                    # Reset the password if the user confirms
                    dialog.destroy()
                    hp.on_reset_password()
                else:
                    dialog.destroy()
            else:
                # If no password exists, prompt to set a new password
                SetPasswordWindow(PASSWORD_FILE)
        else:
            # Disable or clear the password if the checkbox is unchecked
            if os.path.exists(PASSWORD_FILE):
                dialog = Gtk.MessageDialog(
                    parent=None,
                    modal=True,
    ...
    def on_tray_icon_click(self, reason):
        """Handle the tray icon click to restore the app."""
    ...
    def delete_cache_and_cookies(self):
        ...
        # Get the paths to cache and persistent storage
        ...
        # Delete persistent storage directory (cookies, history, etc.) if it exists
    ...
    def handle_notification(self, notification: QWebEngineNotification):
        # Extract the title (sender name) and icon URL from the notification object
        ...
        # Use 'notify-send' with normal urgency and specify the icon (download if necessary)
    ...
    def OpenClickedLink(self):
        ...
        """
        # Execute the JavaScript only once to set up the click listener
        self.browser_widget.page().runJavaScript(js)

        # Periodically check if a link has been clicked and handle it
...
def main():
    if os.path.exists(PASSWORD_FILE):
        # Create the login window and pass the callback to start the Qt application
        login = LoginWindow(PASSWORD_FILE, start_qt_application)
        Gtk.main()
    else:
